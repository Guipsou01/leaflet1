...
async function resizeImage(url, l, data) {
  ...
  return new Promise((resolve) => {
    ...
    try{
      ...
      image2.onerror = () => {
        console.log("[Transform erreur] " + data.titre + ", " + url + ", remplacement par unfond img");
        retour = unfound_img;
        resolve(retour);
      };
    }
...
function affichepopupobjet(data){
  try{
    ...
    //texte += "Layer: " + plan + "<br>";
    if(vPos.getPo3() != null) if(vPos.getPo().getData() == null) {
      texte = null;
      return texte;
    }
...
async function updatePosOnLLObj(obj){
    ...
    if(data.type == IMAGE) await changeUrlSiMM(obj);
  } catch(error) {
    console.error("Error:", error);
    throw error;
  }
}
/**change l'url de l'image en image mipmapé ou inversement*/
async function changeUrlSiMM(obj) {
  if (!leaflet.gestionMipmapViaChangementURL) return;
  const isShort = await leaflet.isShort(obj);
  // HD → normal
  if (obj._data.mipmapActif && !isShort) {
    const url = obj._data.url;
    await new Promise((resolve) => {
      const img = new Image();
      img.src = url;
      img.onload = () => {
        obj.setUrl(url);
        obj._data.mipmapActif = false;
        resolve();
      };
      img.onerror = () => resolve(); // fallback safe
    });
  }

  // normal → mipmap
  else if (!obj._data.mipmapActif && isShort) {
    const urlmm = obj._data.urlmm;
    obj.setUrl(urlmm);
