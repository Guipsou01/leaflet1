...
//MARKER: donnees = cmd,x,y,lx,ly,url,desc
//IMAGE: donnees = cmd,p1x,p1y,p2x,p2y,p3x,p3y,p4x,p4y,pRefx,pRefy,url,title,author,website,imageSizeX,imageSizeY,imageLx,imageLy,isMipmap,angle
  ...
  try{
    ...
    for(var rang = 0; rang < objlistGoogle.length; rang++) {
      ...
      texteCharg.innerHTML = `Remplissage du contenu des listes Leaflet: ${rang} / ${objlistGoogle.length - 1}`;
        ...
        if(ligneptee[0] == TILEMAP_DEFAULT){
          objet = await generateTile(ligneptee);
        }
        else if(ligneptee[0] == MARKER){
...
async function calculTracabilite(rg) {
  try{
    ...
    else if(lignes[0] == MARKER_STATIC_MS){
...
async function objectPosInScreen(data) {
  ...
  else if(data[0] == MARKER_STATIC_MS){
...
async function getLayerObjByData(data){
  ...
  for(var i = 0; i < objListLeaflet.length; i++){
    if(await isTwoLayerSimilarContent(objListLeaflet[i][0],data,true) == true) retour = objListLeaflet[i][1];
    return retour;
  }
  return retour;
}
  ...
  return new Promise((resolve) => {
    switch(lignes[0]){
      ...
      case 'IMG-LX-CENTER':
        (function(){
          ...
          imagePtee.onload = function() {
            ...
            //IMAGE: donnees = cmd,p1x,p1y,p2x,p2y,p3x,p3y,p4x,p4y,pRefx,pRefy,url,title,author,website,imageSizeX,imageSizeY,imageLx,imageLy,isMipmap,angle
            objlistGoogle.push([IMAGE,x1 - lx/2,y1 - ly/2,x1 + lx/2,y1 - ly/2,x1 + lx / 2,y1 + ly / 2,x1 - lx/2,y1 + ly/2,x1,y1,lignes[6],lignes[7],lignes[8],lignes[9], imagePtee.width, imagePtee.height, lx, ly,false,null]);
            resolve();
          };
      ...
      case 'IMG-LX-CENTER-R':
        (function(){
          ...
          imagePtee.onload = async function() {
            ...
            objlistGoogle.push([IMAGE,tabl[0],tabl[1],tabl[2],tabl[3],tabl[4],tabl[5],tabl[6],tabl[7],tabl[8],tabl[9],lignes[6],lignes[7],lignes[8],lignes[9], imagePtee.width, imagePtee.height, lx, ly, false, angle]);
      ...
      case 'IMG-LX':
        (function(){
          ...
          imagePtee.onload = async function() {
            ly = imagePtee.height / imagePtee.width * lx;
            ly = ly;
            objlistGoogle.push([IMAGE,x,y,x + lx,y + ly,imagePtee.src,""]);
            resolve();
          }
        }());
      break;
      case 'IMG-PLR':
        ...
        //
        objlistGoogle.push([IMAGE,x1 + x3modif + x2modif,y1 + y3modif + y2modif,x1 + x3modif,y1 + y3modif,x1 + x2modif,y1 + y2modif,"https://media.tenor.com/ejmDdRGqKDUAAAAe/terminal-montage-donkey-kong.png",0]);
        resolve();
        //
      break;
      default:
...
/**retourne un tableau de 10 val représentant les 5 points de l'image: p1x,p1y,p2x,p2y,p3x,p3y,p4x,p4y,pRefx,pRefy depuis le point central de l'image, ces proportions et son angle final*/
async function getPosApresRotation(x1, y1, lx, ly, angle){
  ...
  return [x1F, y1F, x2F, y2F, x3F, y3F, x4F, y4F, x1, y1];
}
...
function isTwoLayerSimilarContent(layer1, layer2, isAbsoluteTest){
  ...
  else if(layer1[0] == IMAGE && layer2[0] == IMAGE){
  ...
  else if(layer1[0] == MARKER_STATIC_MS && layer2[0] == MARKER_STATIC_MS){
    return true;
  }
  else return false;
}
